...
-- Veracross "Operating Gift History Export" CSV (~185k rows, all years).
-- Server-only (RLS on, no client policy; all access via supabaseAdmin).
-- Applied to the live SAR Supabase project on 2026-06-16.
...
CREATE INDEX IF NOT EXISTS giving_history_cache_activity_idx
...
CREATE INDEX IF NOT EXISTS giving_history_cache_fiscal_year_idx
  ON giving_history_cache(workspace_id, fiscal_year);
