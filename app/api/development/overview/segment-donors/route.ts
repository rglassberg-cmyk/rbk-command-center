...
export async function GET(request: NextRequest) {
  ...
  // 2. Aggregate per constituent. Pools are kept separate so the twin-
  // matching gap-fill rule can be applied after the pass (a soft credit's
  // twin status depends on the donor's full set of type-1 gift ids — see
  // the overview route's scope note for the full rationale).
  // type1 sums ALL type-1 gift amounts (so a donor with 11 gifts shows
  // their total, not one). `type1GiftIds` collects each type-1 row's
  // record `id` (type-1 rows carry hard_credit_gift_id = NULL; the twin
  // soft credit's hcid points to the type-1 gift's own id). Soft credits
  // dedup by hard_credit_gift_id (sc 1 + 2 rows share an id) into
  // `softByKey`, each keeping {hcid, amount} so the after-loop twin test
  // can drop only the soft credits whose hcid is one of the donor's own
  // type-1 ids; multiple same-amount soft credits are NOT collapsed (the
  // old amount-dedup wrongly showed Cory Greenbaum's 11×$540 as $540).
  ...
  for (const g of gifts) {
    ...
    if (g.gift_type === GIFT_TYPE_DONATION) { entry.type1 += Number(g.amount || 0); entry.type1GiftIds.add(g.id); entry.hasType1 = true; type1GiftToConstituent.set(g.id, cid); }
    else if (g.gift_type === GIFT_TYPE_PLEDGE) { entry.pledged += Number(g.pledge_balance || 0); entry.hasType2 = true; type1GiftToConstituent.set(g.id, cid); }
    else if (isSoft) {
      const softKey = g.hard_credit_gift_id != null ? String(g.hard_credit_gift_id) : `amt_${Number(g.amount || 0)}`;
