...
//
// SCHOOL YEAR ACTIVE — re-enabled 2026-09-02 (summer shutdown 2026-07-25 → 2026-09-02).
...
//
// SCHOOL YEAR ACTIVE — re-enabled 2026-09-02.
// ---------------------------------------------------------------------------
...
async function runGiftsSyncForAllWorkspaces() {
    ...
    const targetWorkspaces = (workspaces || []).filter((w) => { var _a; return ((_a = w.modules) === null || _a === void 0 ? void 0 : _a.development) === true; });
    console.log('[GIFTS SYNC] Found', targetWorkspaces.length, 'workspaces with development module');
    for (const workspace of targetWorkspaces) {
        try {
            const response = await (0, node_fetch_1.default)('https://ssrrbkcmdcenter-429508710310.us-east1.run.app/api/development/sync-gifts-internal', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'X-Internal-Secret': process.env.INTERNAL_SYNC_SECRET || '',
        ...
        try {
            const response = await (0, node_fetch_1.default)('https://ssrrbkcmdcenter-429508710310.us-east1.run.app/api/development/sync-constituents-internal', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'X-Internal-Secret': process.env.INTERNAL_SYNC_SECRET || '',
...
// SCHOOL YEAR ACTIVE — both re-enabled 2026-09-02.
// Both drive runGiftsSyncForAllWorkspaces() above, which syncs gifts AND then
// chains the constituents sync (/api/development/sync-constituents-internal).
// There is deliberately NO separate constituents scheduler — restoring these
// two restores constituent syncing as well.
//
// ⚠️ TIMEOUT (added 2026-09-02, on restart): these MUST override the 1st-gen
// default 60s timeout. `runGiftsSyncForAllWorkspaces()` calls the gifts sync
// endpoint and waits, then chains the constituents sync and waits again. On live
// SAR data the gifts leg alone takes ~58s and the constituents leg ~3min, so at
// 60s the function was being killed mid-run: the hourly job frequently died
// before the gifts sync returned, and the chained constituents sync NEVER
// completed — `constituents_cache` had not refreshed since 2026-07-25, which
// silently staled the roles/grades behind the Development Overview segment
// classification. 540s is the 1st-gen maximum and comfortably covers both legs
// plus the Cloud Run 300s request ceiling.
exports.syncGiftsHourlyWeekdays = functions
...
exports.syncGiftsDailyWeekends = functions
    .runWith({ timeoutSeconds: 540 })
    .pubsub
    .schedule('0 6 * * 0,6')
    .timeZone('America/New_York')
...
function isForwardingStub(text) {
 ...
 */
function collectParts(payload, plains, htmls) {
    var _a, _b;
    if (payload.mimeType === 'text/plain' && ((_a = payload.body) === null || _a === void 0 ? void 0 : _a.data)) {
        plains.push(decodeBase64Url(payload.body.data));
    }
    else if (payload.mimeType === 'text/html' && ((_b = payload.body) === null || _b === void 0 ? void 0 : _b.data)) {
...
function parseEmailBody(payload) {
    ...
    if (!payload.parts && ((_a = payload.body) === null || _a === void 0 ? void 0 : _a.data)) {
...
function getHeader(headers, name) {
    ...
    return (h === null || h === void 0 ? void 0 : h.value) || '';
}
// ---------------------------------------------------------------------------
...
const triageGmailHandler = (async () => {
    ...
    for (const ws of workspaceRows) {
        ...
        try {
            ...
            const listRes = await (0, node_fetch_1.default)(listUrl, {
            ...
            for (const { id: msgId } of messageIds) {
                try {
                    ...
                    const msgRes = await (0, node_fetch_1.default)(msgUrl, {
                        headers: { Authorization: `Bearer ${accessToken}` },
                    });
...
IMPORTANT: If you set action_needed to any variation of "no reply needed", "no action required", "no response needed", or similar, you MUST set priority to "fyi", not "owner_action".`,
                    ...
                    const triageText = ((_c = (_b = triageResponse.choices[0]) === null || _b === void 0 ? void 0 : _b.message) === null || _c === void 0 ? void 0 : _c.content) || '';
                    let triage;
                    try {
                    ...
                    const { error: insertError } = await supabase
                        .from('emails')
                        .insert(Object.assign({ workspace_id: WORKSPACE_ID, message_id: gmailMessageId, thread_id: gmailThreadId, from_name: fromName || null, from_email: fromEmail, subject, received_at: new Date(dateStr).toISOString(), body_text: bodyText, summary: triage.summary, action_needed: triage.action_needed || null, draft_reply: triage.draft_reply || null, priority: triage.priority, assigned_to: assignedTo, status: 'pending', is_unread: true }, (isShiva ? { action_status: 'shiva' } : {})));
                    if (insertError) {
                    ...
                    if (labelName) {
                        try {
                            ...
                            if (labelId) {
                                await (0, node_fetch_1.default)(`https://gmail.googleapis.com/gmail/v1/users/${encodeURIComponent(owner_email)}/messages/${msgId}/modify`, {
                                    method: 'POST',
                                    headers: {
...
// Label helpers
// ---------------------------------------------------------------------------
async function getOrCreateLabel(accessToken, userEmail, labelName, cache) {
    if (cache[labelName])
    ...
    const createRes = await (0, node_fetch_1.default)(listUrl, {
        method: 'POST',
        headers: {
            Authorization: `Bearer ${accessToken}`,
            'Content-Type': 'application/json',
...
const syncDraftsReadyHandler = (async () => {
    ...
    for (const ws of workspaceRows) {
        ...
        try {
            ...
            const draftsRes = await (0, node_fetch_1.default)(draftsUrl, {
                headers: { Authorization: `Bearer ${accessToken}` },
            });
            ...
            for (const [threadId, msgIds] of threadMap) {
                try {
                    ...
                    const threadRes = await (0, node_fetch_1.default)(threadUrl, {
                        headers: { Authorization: `Bearer ${accessToken}` },
                    });
                    const threadData = await threadRes.json();
                    if (!threadData.error && threadData.messages) {
                        const hasSentMessage = threadData.messages.some((m) => m.labelIds && m.labelIds.includes('SENT'));
                        if (hasSentMessage) {
                            skippedAlreadySent++;
                            await supabase
                                .from('emails')
                    ...
                    const draftRes = await (0, node_fetch_1.default)(draftUrl, {
                        headers: { Authorization: `Bearer ${accessToken}` },
                    });
                    ...
                    const draftBody = parseEmailBody((_a = draftData.message) === null || _a === void 0 ? void 0 : _a.payload);
                    if (!draftBody) {
                        skippedNoDraft++;
                        await swapDraftsReadyLabel(accessToken, owner_email, msgIds, draftsReadyLabelId, labelCache);
                        continue;
                    }
                    const draftHeaders = ((_c = (_b = draftData.message) === null || _b === void 0 ? void 0 : _b.payload) === null || _c === void 0 ? void 0 : _c.headers) || [];
...
async function swapDraftsReadyLabel(accessToken, userEmail, msgIds, draftsReadyLabelId, labelCache) {
    const draftsSyncedLabelId = await getOrCreateLabel(accessToken, userEmail, 'Drafts Synced', labelCache);
    for (const msgId of msgIds) {
        try {
            await (0, node_fetch_1.default)(`https://gmail.googleapis.com/gmail/v1/users/${encodeURIComponent(userEmail)}/messages/${msgId}/modify`, {
                method: 'POST',
                headers: {
                    Authorization: `Bearer ${accessToken}`,
                    'Content-Type': 'application/json',
